package tramais.hnb.hhrfid.litePalBean

import org.litepal.crud.LitePalSupport

class FarmListCache : LitePalSupport() {
    var number: String? = null
    var name: String? = null
    var category: String? = null
    var area: String? = null
    var zjCategory: String? = null
    var zjNumber: String? = null
    var mobile: String? = null
    var sFZAddress: String? = null
    var raiseAddress: String? = null
    var bankName: String? = null
    var accountNumber: String? = null
    var accountName: String? = null
    var creatTime: String? = null
    var idPath: String? = null
    var bankPath: String? = null
    var remark: String? = null
    var upDateTime: String? = null
    var zjBackPicture: String? = null
    var isPoor: String? = null
    var singPic: String? = null
    var singTime: String? = null
    var isSingUp: String? = null
    var isUpLoad: String? = null
    var overdueTime: String? = null
    var fRegionNumber: String? = null
    var FBankCode: String? = null
    var FBankRelatedCode: String? = null
    var fStartime: String? = null
    var natureCode: String? = null
    var natureName: String? = null
    var comNumber: String? = null
    var empNumber: String? = null
}